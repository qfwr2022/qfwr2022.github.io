...
image: /slides/lec-05/imgs/logo-dplyr.png
# editor: visual
---

<hr>

在上一讲中我们用两次课的时间学习 R 语言编程方面的核心知识：**向量**与**函数**，为接下来探索“净土宇宙 🌌”奠定了知识基础。“净土宇宙 🌌”的核心数据结构并非原子型向量或普通列表对象，而是 `tibble`（由行（样本）和列（变量）构成的表格型数据），dplyr 包基于**数据操作语法**构建出一组用来操作 `tibble` 对象的核心函数——它们就是我们在接下来两次课中需要学习、理解并掌握的知识。
...
📖 阅读 [*R for Data Science*](https://r4ds.had.co.nz/) 一书**第5章**、**第13章**和**第18章**，这些章节依次介绍如何利用 dplyr 包中的核心函数对单表数据和关系型数据表进行处理以及管道操作符 `%>%` 的相关知识。
